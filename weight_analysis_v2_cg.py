...
def plt_unit2(axi, mat, weight_mask, sorted_id=None):
    buffer = mat.copy()
    if weight_mask.sum() == 0:
        buffer = None
    else:
        min_val = mat[weight_mask].min()
...
for weight_mask, idx in zip(
    ...
    for band in filter_pool:
        ...
        tdmi_data_cg[band] = tdmi_data_cg_band.copy()
        
        tdmi_mask[band] = (tdmi_data_cg_band > tdmi_sep[band][idx])*(tdmi_data_cg_band <= tdmi_sep[band][idx+1])
        print(tdmi_mask[band].sum())

    plt_unit2(ax[0,0], weight, weight_mask, sorted_id)
    ax[0,0].set_title('Weight Matrix')
    indices = [(1,0),(2,0),(3,0),(1,1),(2,1),(3,1),(0,1),]
    for index, band in zip(indices, filter_pool):
        plt_unit2(ax[index], tdmi_data_cg[band], tdmi_mask[band], sorted_id)
    ...
    plt.tight_layout()
    plt.savefig(path + f'weight_analysis_v2_{idx:d}_cg.png')
    plt.close()
