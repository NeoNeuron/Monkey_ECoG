...
G = nx.DiGraph(1./con_2d).to_undirected()
...
gen_sc_fc_figure_new(ax[1], fc_gc[band], 1./shortest_path_length_array[roi_mask], sc_gc[band]!=1.5)
...
# %%
fig, ax = plt.subplots(1,3, figsize=(13,4), dpi=400)
band = 'raw'
...
gen_sc_fc_figure(ax[2], diff, 1./(sc_tdmi[band]*shortest_path_length_array[roi_mask]), new_mask, c=diff, is_log='x')
ax[2].set_xlabel('Indirect path factor')
ax[2].set_ylabel('Diff. (FC-SC)')

@spines_formater
@axis_log_formater(axis='y')
@axis_log_formater(axis='x')
def gen_sp_p(ax):
    cax = ax.scatter(np.log10(1./sc_tdmi[band])[new_mask], np.log10(shortest_path_length_array[roi_mask])[new_mask], 
...
fig, ax = plt.subplots(1,2,dpi=400, figsize=(10,4))
vmax = np.abs(diff).max()
cax = ax[0].pcolormesh(diff_mat_reorder, vmax=vmax, vmin=-vmax, cmap=plt.cm.RdBu_r)
plt.colorbar(cax, ax=ax[0])
ax[0].axis('scaled')
ax[0].invert_yaxis()
...
ax[1].scatter(loc[:, 0], loc[:, 1], s=40, lw=0, c=diff_order, cmap=plt.cm.RdBu_r)
ax[1].plot((100, 100), (1050,1150), 'k')
ax[1].plot((100, 200), (1150,1150), 'k')
ax[1].text(0.1, 0.1, '100', transform=ax[1].transAxes)
ax[1].axis('equal')
ax[1].invert_yaxis()
ax[1].axis("off")
...
def gen_sc_fc_figure(*args, **kwargs):
    return gen_sc_fc_figure_new.__wrapped__.__wrapped__(*args, **kwargs)
gen_sc_fc_figure(ax[2], diff, 1./(sc_gc[band]*shortest_path_length_array[roi_mask]), c=diff, is_log='x')
ax[2].set_xlabel('Indirect path factor')
ax[2].set_ylabel('Diff. (FC-SC)')
...
@axis_log_formater(axis='x')
def gen_sp_p(ax):
    cax = ax.scatter(np.log10(1./sc_gc[band]), np.log10(shortest_path_length_array[roi_mask]), 
