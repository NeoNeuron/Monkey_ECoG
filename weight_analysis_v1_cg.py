...
def plt_unit2(axi, mat, weight_mask, sorted_id=None):
    buffer = mat.copy()
    min_val = np.nanmin(mat[weight_mask])
...
for weight_mask, idx in zip(
    ...
    plt.tight_layout()
    plt.savefig(path + f'weight_analysis_v1_{idx:d}_cg.png')
    plt.close()
